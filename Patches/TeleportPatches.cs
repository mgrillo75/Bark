    ...
    internal class TeleportPatch
    {
        ...
        internal static bool Prefix(Player __instance)
        {
            try
            {
                if (_isTeleporting)
                {
                    if (_killVelocity) __instance.SetPlayerVelocity(Vector3.zero);

                    if (_rotate) __instance.Turn(_teleportRotation - Camera.main.transform.rotation.eulerAngles.y);

                    __instance.TeleportTo(_teleportPosition, Quaternion.AngleAxis(_teleportRotation, Vector3.up), true, false);
